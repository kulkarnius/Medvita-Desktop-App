...
#Request the Temperature from the device
treq = 'T'

#Convert string requests to bytes
def to_bytes(input):
    byte = input.encode()
    return byte

#Define request
request = to_bytes(treq)
...
"""
Code picked up from Ryan's table generator for our specific sensor. It gives us the temperature reading. The computer has enough processing power to not need the lookup table.
Written by: Ryan Mah
